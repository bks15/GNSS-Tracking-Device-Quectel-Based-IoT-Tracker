...
void AT(String cmd, String test) {
  while (!getResponse(F(cmd), test, 1000));
}
...
bool connectGSM(void)
{
  ...
  */
  AT(F("AT+CGDCONT=1,\"IP\",\"airtelgprs.com\""), OK);
  ...
  */
  AT(F("AT+QSSLCFG=\"seclevel\",0,1"), OK);
  ...
  */
  ...
  /*
    Delete the File(s) in the Storage Medium
      AT+QFDEL=<filename>
  */
  AT(F("AT+QFDEL=\"UFS:ca.pem\""), OK);
  // GNSS
...
void openMQTTConnection() {
  ...
  */
  AT(F("AT+QMTOPEN=0,\"6a99624836bd49aeaff2cd08268824d0.s1.eu.hivemq.cloud\",8883"), OK);

  /*
    Connect a Client to MQTT Server
      AT+QMTCONN=<client_idx>,<clientID>[,<username>,<password>]
  */
  AT(F("AT+QMTCONN=0,\"client001\",\"bksan\",\"P@ssw0rd\""), OK);
